...
- Robust headless backend constructed using Spring Boot MVC.
- Directly interacts with PhantomBuster Agents to launch searches and retrieve structured JSON results.
- Utilizes Spring Data JPA to store and retrieve historical searches efficiently from a PostgreSQL database.
- Proper application layer separation (Controllers, Services, Repositories, Client, DTOs).
- Comprehensive test suite written in JUnit 5 with Mockito for TDD adherence.
- Unified API response format using `RestControllerAdvice`.
